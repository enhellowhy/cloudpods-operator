...
func SetDefaults_OnecloudClusterSpec(obj *OnecloudClusterSpec, isEE bool) {
	...
	for cType, spec := range map[ComponentType]*hyperImagePair{
		...
		WorkflowComponentType:        nHP(&obj.Workflow.DeploymentSpec, useHyperImage),
		MeterLIComponentType:         nHP(&obj.MeterLI.DeploymentSpec, useHyperImage),
...
func setDefaults_Components_ServicePort(obj *OnecloudClusterSpec) {
	...
	for _, spec := range []*serviceSpecPair{
		...
		newSP(&obj.Workflow.Service, constants.WorkflowPort),
		newSP(&obj.MeterLI.Service, constants.MeterLIPort),
...
func SetDefaults_OnecloudClusterConfig(obj *OnecloudClusterConfig) {
	...
	for opt, tmp := range map[*ServiceDBCommonOptions]userDBPort{
		...
		&obj.MeterLI:                             {constants.MeterLIAdminUser, constants.MeterLIPort, constants.MeterLIDB, constants.MeterLIDBUser},
